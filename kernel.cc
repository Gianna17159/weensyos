...
static void fork_free(x86_64_pagetable * pt);
...
void process_setup(pid_t pid, const char* program_name) {
    ...
    vmiter proc_it(proc_pagetable, 0);

    for (vmiter kern_it(kernel_pagetable, 0); kern_it.va() < PROC_START_ADDR;) {
       
       
        proc_it.map(kern_it.pa(), kern_it.perm());
    ...
    for (loader.reset(); loader.present(); ++loader) {
        for (uintptr_t a = round_down(loader.va(), PAGESIZE);
            ...
            if (!pa) {
               fork_free(proc_pagetable);
               current->state = P_FREE;
                return;
            }
             memset((void*) pa, 0, PAGESIZE);
            vmiter user_it(proc_pagetable, a);
            if (loader.writable()) {
                user_it.map(pa, PTE_P | PTE_W | PTE_U);
            } else {
                 user_it.map(pa, PTE_P | PTE_U);
            }
        }
    }
    ...
    for (loader.reset(); loader.present(); ++loader) {
        vmiter it(proc_pagetable, loader.va());
    ...
    memset((void*) pa, 0, PAGESIZE);
    vmiter user_it(proc_pagetable, stack_addr);
    user_it.map(pa, PTE_P | PTE_W | PTE_U);
...
int syscall_page_alloc(uintptr_t addr) {
    ...
    it.map(pa, PTE_P | PTE_W | PTE_U);
    memset((void*) pa, 0, PAGESIZE);
    return 0;
}
void fork_free(x86_64_pagetable * pt) {
    for (vmiter it(pt); it.va() < MEMSIZE_VIRTUAL; it += PAGESIZE) {
        if (it.pa() != CONSOLE_ADDR && (it.perm() & PTE_P) == PTE_P && (it.perm() & PTE_U) == PTE_U) {
            kfree((void *) it.pa());
        }
    }
...
pid_t syscall_fork() {
    ...
    memset((void *) child_pagetable, 0, PAGESIZE);
    ptable[pid].pagetable = child_pagetable;

    vmiter ch_it(child_pagetable);
    for (vmiter par_it(current->pagetable); par_it.va() < MEMSIZE_VIRTUAL; par_it+=PAGESIZE) {
        if (par_it.va() < PROC_START_ADDR || (par_it.perm() & PTE_U) != PTE_U
                || (par_it.perm() & PTE_W) != PTE_W) {
            if (ch_it.try_map(par_it.pa(), par_it.perm()) == -1) {
               
                fork_free(child_pagetable);
                return -1;
            }
            if ((par_it.perm() & PTE_W) != PTE_W && (par_it.perm() & PTE_U) == PTE_U) {
                pages[par_it.pa() / PAGESIZE].refcount++;
            }
        }
        else {
            uintptr_t pa = (uintptr_t) kalloc(PAGESIZE);
            ...
            memset((void *) pa, 0, PAGESIZE);
            memcpy((void *) pa, (void *) par_it.pa(), PAGESIZE);
            
            if (ch_it.try_map(pa, par_it.perm()) == -1) {
                kfree((void *) pa);
                fork_free(child_pagetable);
                return -1;
            }
